...
var Tid = null, turnN=1;
var Rotates = new Array();
function initnotscrambled(){
    Rotates = []; turnN=1; clearTimeout(Tid);
    $("#turn").html("&nbsp;"); $("#rotate").html("&nbsp;");
    for(a[0]=0,j=0;6>j;j++)for(i=1;10>i;i++)a[i+9*j]=j+1;
    -1==String(document.domain).indexOf("ruwix")&&(a=0),kiir();
...
function quickIn() {
    ...
    RotatesText = window.prompt("Input rotation symbols split by comma or space (xx2:twice)", clipdt);
...
91==a&&$("#rotLayer").css("transform","rotatez("+10*counter+"deg)"),92==a&&$("#rotLayer").css("transform","rotatez("+-(20*counter)+"deg)"),
